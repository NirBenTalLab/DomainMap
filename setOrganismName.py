...
# UNP Organism data: http://www.uniprot.org/taxonomy/?query=id:{tax_id}&format=tab
...
class MapPDBOrganism(object):
    ...
    def __call__(self, *args, **kwargs):
        pdb_mol = self.get_pdb_molecules(self.tax_id)
        if pdb_mol is not None and 200 == pdb_mol.status_code:
            pdbchain_organism = self.process_pdb_molecules(self.tax_id, pdb_mol.json())
            if pdbchain_organism is not None:
                ...
                print("%s pdbchain_organism is None" %self.tax_id)
        print("%s finished unsuccessfully" % self.tax_id, pdb_mol)
